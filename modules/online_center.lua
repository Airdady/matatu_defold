...
function M.draw(self, ctx)
    ...
    for i = min_i, max_i do
        ...
        if row_cy + row_h/2 >= list_bottom and row_cy - row_h/2 <= list_top then
            ...
            --
            -- Somebody seated at a party has paid an entry and is watching a
            -- twenty-second clock, and the server refuses every request to
            -- them for exactly that reason (isInParty). A row that still
            -- offered a challenge would be a button that cannot work — the
            -- player taps it, waits, and is told no.
            ...
            if playing then
                ...
                -- most twenty seconds, and a player deciding whether to wait
                -- for somebody deserves to know which.
