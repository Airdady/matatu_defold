...
local function label(pos, text, size, color, align, font_name)
    ...
    local base_size = (font_name == "poppins_bold" or font_name == "title") and 36 or 24
...
local function t4_add_grave(self, name, avatar)
    ...
    local nm = label(vmath.vector3(gx, gy - 38, 0), string.upper(tostring(name or "")), 13, vmath.vector4(0.72, 0.74, 0.78, 1), gui.PIVOT_CENTER, "poppins_bold")
    gui.set_xanchor(nm, gui.ANCHOR_RIGHT); gui.set_yanchor(nm, gui.ANCHOR_NONE)
    local tag = label(vmath.vector3(gx + 24, gy + 24, 0), "OUT", 11, vmath.vector4(0.95, 0.40, 0.40, 1), gui.PIVOT_CENTER, "poppins_bold")
...
function M.seat(self, message)
    ...
    if not b then
        ...
        local nm = label(vmath.vector3(x, y - 62, 0), "", 20, C_WHITE, gui.PIVOT_CENTER, "poppins_bold")
        gui.set_xanchor(nm, gui.ANCHOR_NONE); gui.set_yanchor(nm, gui.ANCHOR_NONE)
        local tot = label(vmath.vector3(x, y - 86, 0), "", 18, vmath.vector4(1, 0.84, 0.2, 1), gui.PIVOT_CENTER, "poppins_bold")
...
function M.chamber_init(self, message)
    ...
    local title = label(vmath.vector3(125, -24, 0), "SCORE CAP  " .. tostring(threshold), 16, vmath.vector4(1, 0.84, 0.2, 1), gui.PIVOT_CENTER, "poppins_bold")
    ...
    for i, r in ipairs(rows) do
        ...
        local rk = label(vmath.vector3(14, ry, 0), tostring(i), 14, vmath.vector4(1, 0.84, 0.2, 1), gui.PIVOT_W, "poppins_bold"); gui.set_parent(rk, root)
        local nm = label(vmath.vector3(34, ry, 0), string.upper(tostring(r.name or "")), 14, C_WHITE, gui.PIVOT_W, "poppins_bold"); gui.set_parent(nm, root)
        local val = label(vmath.vector3(236, ry, 0), tostring(total), 16, C_WHITE, gui.PIVOT_E, "poppins_bold"); gui.set_parent(val, root)
